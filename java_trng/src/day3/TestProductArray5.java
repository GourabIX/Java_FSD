...
public class TestProductArray5 {
	...
	public static void main(String[] args)
	{
		...
		// displayCheapestProductDetails(prod_arr);
		displayProductsWhoseNameStartsWith(prod_arr, "D");
	...
	public static void displayCheapestProductDetails(Product[] prod_arr)
	{
		int lowest_price = prod_arr[0].getPrice();
		Product cheapest_prod = new Product();
		for(Product prod_obj: prod_arr)
		{
			if (prod_obj.getPrice() < lowest_price)
			{
				lowest_price = prod_obj.getPrice();
				cheapest_prod = prod_obj;
			}
		}
		System.out.println("\n" + cheapest_prod.toString());
	}
	
	public static void displayProductsWhoseNameStartsWith(Product[] prod_arr, String c)
	{
		for(Product prod_obj: prod_arr)
		{
			if (prod_obj.getProductName().startsWith(c)) 
			{
